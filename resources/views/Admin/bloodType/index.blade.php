@extends('layouts.app')
@section('title','All of Blood Types')
@section('page')
    <a href="{{url(route('bloodType.index'))}}">Blood Types</a>
    @endsection
@section('content')
    ...
    <div class="col-sm-12"><table id="example2" class="table table-bordered table-hover dataTable dtr-inline collapsed" role="grid" aria-describedby="example2_info">
            <a class="btn btn-primary" href="{{(route('bloodType.create'))}}"><i class="fa fa-plus"></i> Add new blood type </a>
            <thead>
            <tr role="row">
            ...
            <tbody>
            @foreach($bloods as $go)
                <tr role="row" class="odd">
                    <td class="sorting_1" tabindex="0">{{$go->id}}</td>
                    <td class="">{{$go->name}}</td>
                    <td class="text-center">
                        <a href="{{url(route('bloodType.edit',$go->id))}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                    </td>
                    <td class="text-center">
                        <form action="{{url(route('bloodType.destroy',$go->id))}}" method="post">
                            @method('DELETE')
                            @csrf
